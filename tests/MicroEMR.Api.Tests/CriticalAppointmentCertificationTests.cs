...
public sealed class CriticalAppointmentCertificationTests
{
    ...
    public void ManifestAppendsOnlyNextMigration()
    {
        ...
        Assert.Equal("0042-scheduling-critical-appointments", migrationIds[^6]);
        Assert.Equal("0043-patient-chart-read-audit", migrationIds[^5]);
        Assert.Equal("0044-structured-read-audit-procedure", migrationIds[^4]);
        Assert.Equal("0045-structured-disclosure-audit-events", migrationIds[^3]);
        Assert.Equal("0046-aggregate-report-audit-events", migrationIds[^2]);
        Assert.Equal("0047-patient-immunization-history", migrationIds[^1]);
