...
public sealed class PatientPrescriptionFoundationTests
{
 ...
 [Fact]public void Migration0050RemainsExactlyOnceAndIsFollowedBy0051(){using var d=JsonDocument.Parse(File.ReadAllText(Path.Combine(Root(),"db","tenant-clinical","manifest.json")));var ids=d.RootElement.EnumerateArray().Select(x=>x.GetProperty("migrationId").GetString()).ToArray();Assert.Equal("0050-patient-prescription-foundation",ids[^3]);Assert.Equal("0051-result-review-acknowledgement-hardening",ids[^2]);Assert.Equal("0052-cds-foundation",ids[^1]);Assert.Equal(1,ids.Count(x=>x=="0050-patient-prescription-foundation"));Assert.False(File.Exists(Path.Combine(Root(),"db","tenant-clinical","migrations","0051-patient-prescription-foundation.sql")));}
