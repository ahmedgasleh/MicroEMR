...
public sealed class PatientPrescriptionFoundationTests
{
 ...
 [Fact]public void Migration0050RemainsExactlyOnceAndIsFollowedBy0051(){using var d=JsonDocument.Parse(File.ReadAllText(Path.Combine(Root(),"db","tenant-clinical","manifest.json")));var ids=d.RootElement.EnumerateArray().Select(x=>x.GetProperty("migrationId").GetString()).ToArray();Assert.Equal("0050-patient-prescription-foundation",ids[^9]);Assert.Equal("0051-result-review-acknowledgement-hardening",ids[^8]);Assert.Equal("0052-cds-foundation",ids[^7]);Assert.Equal("0053-cdm-enrollment-foundation",ids[^6]);Assert.Equal("0054-results-provenance-correction-foundation",ids[^5]);Assert.Equal(1,ids.Count(x=>x=="0050-patient-prescription-foundation"));Assert.False(File.Exists(Path.Combine(Root(),"db","tenant-clinical","migrations","0051-patient-prescription-foundation.sql")));}
