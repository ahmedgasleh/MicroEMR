...
BEGIN
    ...
    SET @ReportKey = NULLIF(LTRIM(RTRIM(@ReportKey)), N'');

    DECLARE @IsAggregateReport BIT = CASE WHEN
        @EventType IN (N'ReportExecuted', N'CsvExported')
        AND @ResourceType = N'Report'
        THEN 1 ELSE 0 END;
    ...
    (
        ...
        OR @IsAggregateReport = 1
    )
        THROW 52210, 'Unsupported structured read audit event/resource combination.', 1;

    DECLARE @PatientId BIGINT = NULL;

    IF @IsAggregateReport = 1
    BEGIN
        IF @PatientUid IS NOT NULL
            THROW 52216, 'Aggregate report audit events cannot identify one patient.', 1;
        IF @ResourceUid IS NOT NULL
            THROW 52217, 'Aggregate report audit events use a governed report key, not a resource UID.', 1;
        IF @ReportKey <> N'AppointmentStatusDateReport' OR @ReportKey IS NULL
            THROW 52218, 'Unsupported aggregate report identity.', 1;
    END
    ELSE
    BEGIN
        IF @ReportKey IS NOT NULL
            THROW 52219, 'Patient-scoped audit events cannot include a report identity.', 1;
    ...
    (
        ...
        CASE WHEN @IsAggregateReport = 1 THEN @ReportKey ELSE CONVERT(NVARCHAR(100), @ResourceUid) END,
