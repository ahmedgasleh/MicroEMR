...
public sealed class SqlPlatformMembershipAdministrationService : IPlatformMembershipAdministrationService
{
    ...
    private async Task<IReadOnlyList<PlatformMembershipInfo>> QueryAsync(string procedure, string name, object value, CancellationToken token)
    {
        ...
        {
            var k = (r.GetString(0), r.GetGuid(1));
            if (!builders.TryGetValue(k, out var b))
            {
                var hasLifecycleColumns = r.FieldCount == 9;
                if (!hasLifecycleColumns && r.FieldCount != 7)
                    throw new InvalidOperationException("The membership query returned an unexpected result shape.");

                b = (r.GetString(2), r.GetString(3), r.GetString(4), r.GetBoolean(5), [],
                    hasLifecycleColumns && !r.IsDBNull(7)
                        ? new DateTimeOffset(DateTime.SpecifyKind(r.GetDateTime(7), DateTimeKind.Utc))
                        : null,
                    hasLifecycleColumns ? Convert.ToBase64String((byte[])r[8]) : null);
            }
            if (!r.IsDBNull(6)) b.Roles.Add(r.GetString(6));
            builders[k] = b;
        }
