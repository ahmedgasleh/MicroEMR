...
public sealed class SqlAccessProfileRepository(IConfiguration configuration) : IAccessProfileRepository
{
    ...
    private async Task ExecuteAsync(string name,CancellationToken token,params (string,SqlDbType,object)[] values){await using var c=new SqlConnection(connectionString);await using var cmd=Command(c,name);foreach(var x in values)Add(cmd,x.Item1,x.Item2,x.Item3);try{await c.OpenAsync(token);await cmd.ExecuteNonQueryAsync(token);}catch(SqlException ex)when(ex.Number is 51303 or 51401){throw new KeyNotFoundException("The tenant membership or access profile was not found.",ex);}catch(SqlException ex)when(ex.Number is 51307 or 51402){throw new InvalidOperationException("The access configuration was changed by another administrator.",ex);}}
    private static SqlCommand Command(SqlConnection c,string n)=>new(n,c){CommandType=CommandType.StoredProcedure};
    private static void Add(SqlCommand c,string n,SqlDbType t,object v){var p=c.Parameters.Add(n,t);if(t==SqlDbType.NVarChar)p.Size=-1;p.Value=v;}
