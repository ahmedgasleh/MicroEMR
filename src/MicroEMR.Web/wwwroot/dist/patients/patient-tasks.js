...
if (patientUid) {
    ...
    function render() {
        if (!tasks.length) {
            root.innerHTML = `<div class="alert alert-light border">${filter.value === "Open" ? "No open tasks recorded." : filter.value === "Completed" ? "No completed tasks recorded." : "No tasks recorded."}</div>`;
            return;
        }
