...
if (patientUid && listRoot && pageMessage && createForm && saveButton && modalMessage && detailsBody && token) {
    ...
    const relevantDate = (referral) => {
        if (referral.closedAtUtc)
            return `Closed ${formatDate(referral.closedAtUtc)}`;
        if (referral.responseReceivedAtUtc)
            return `Response received ${formatDate(referral.responseReceivedAtUtc)}`;
        if (referral.sentAtUtc)
            return `Sent ${formatDate(referral.sentAtUtc)}`;
