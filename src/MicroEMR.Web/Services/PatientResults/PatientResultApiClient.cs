...
public interface IPatientResultApiClient
{
    ...
    Task<IReadOnlyList<PatientResultResponse>> History(Guid patientUid,Guid uid,CancellationToken token);
    Task<PatientResultResponse?> Correct(Guid patientUid,Guid uid,object request,CancellationToken token);
    Task<PatientResultResponse?> EnteredInError(Guid patientUid,Guid uid,object request,CancellationToken token);
}
...
public sealed class PatientResultApiClient(HttpClient client,IHttpContextAccessor contextAccessor):IPatientResultApiClient
{
    ...
    public async Task<IReadOnlyList<PatientResultResponse>>History(Guid p,Guid u,CancellationToken t){using var q=await Request(HttpMethod.Get,$"api/patients/{p}/results/{u}/history");using var r=await client.SendAsync(q,t);r.EnsureSuccessStatusCode();return await r.Content.ReadFromJsonAsync<List<PatientResultResponse>>(cancellationToken:t)??[];}
    public Task<PatientResultResponse?>Correct(Guid p,Guid u,object x,CancellationToken t)=>Mutate(HttpMethod.Post,$"api/patients/{p}/results/{u}/correct",x,t);
    public Task<PatientResultResponse?>EnteredInError(Guid p,Guid u,object x,CancellationToken t)=>Mutate(HttpMethod.Post,$"api/patients/{p}/results/{u}/entered-in-error",x,t);
