...
public sealed class PatientResultApiClient(HttpClient client,IHttpContextAccessor contextAccessor):IPatientResultApiClient
{
    ...
    private async Task<PatientResultResponse?>Mutate(HttpMethod method,string uri,object value,CancellationToken token){using var request=await Request(method,uri);request.Content=JsonContent.Create(value);using var response=await client.SendAsync(request,token);if(response.StatusCode==System.Net.HttpStatusCode.Conflict)throw new HttpRequestException("Reviewed results cannot be edited.",null,response.StatusCode);response.EnsureSuccessStatusCode();return await response.Content.ReadFromJsonAsync<PatientResultResponse>(cancellationToken:token);}
