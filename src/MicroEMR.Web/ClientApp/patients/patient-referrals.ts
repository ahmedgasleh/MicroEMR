...
if (patientUid && listRoot && pageMessage && createForm && saveButton && modalMessage && detailsBody && token) {
  ...
  function renderDetails(referral: ReferralDetails): void {
      ...
      document.querySelector<HTMLButtonElement>("#saveReferralFollowUp")?.addEventListener("click",()=>void saveFollowUp(false));
      document.querySelector<HTMLButtonElement>("#clearReferralFollowUp")?.addEventListener("click",()=>void saveFollowUp(true));
      void loadSupportingDocuments(referral);
  }

  async function saveFollowUp(clear: boolean): Promise<void> {
    if (!selectedReferral) return;
    const input=document.querySelector<HTMLInputElement>("#referralFollowUpDue");
    if (!clear&&!input?.value){showPageMessage("Select a follow-up date and time.","danger");return;}
    const body=new FormData();body.set("PatientUid",patientUid);body.set("ReferralUid",selectedReferral.referralUid);
    body.set("RowVersion",selectedReferral.rowVersion);body.set("FollowUpDueAtUtc",clear?"":new Date(input!.value).toISOString());
    body.set("__RequestVerificationToken",token.value);
    try{const response=await fetch("/PatientReferrals/SetFollowUp",{method:"POST",body});const result=await response.json() as ReferralReply;
      if(!response.ok||!result.success||!result.referral)throw new Error(result.message??"Follow-up could not be changed.");
      selectedReferral=result.referral;renderDetails(selectedReferral);await loadReferrals();showPageMessage(result.message??"Follow-up updated.","success");
    }catch(error:unknown){showPageMessage(error instanceof Error?error.message:"Follow-up could not be changed.","danger");}
  }
  ...
  async function loadSupportingDocuments(referral: ReferralDetails): Promise<void> {
    ...
    try {
      ...
      root.querySelector<HTMLButtonElement>("#saveReferralResponseDocument")?.addEventListener("click",()=>void saveResponseDocument(root));
    } catch (error: unknown) {
  ...
  async function saveResponseDocument(root: HTMLElement): Promise<void> {
    if(!selectedReferral)return;
    const documentUid=root.querySelector<HTMLSelectElement>("#referralResponseDocument")?.value??"";
    const body=new FormData();body.set("PatientUid",patientUid);body.set("ReferralUid",selectedReferral.referralUid);
    body.set("DocumentUid",documentUid);body.set("RowVersion",selectedReferral.rowVersion);body.set("__RequestVerificationToken",token.value);
    try{const response=await fetch("/PatientReferrals/SetResponseDocument",{method:"POST",body});const result=await response.json() as ReferralReply;
      if(!response.ok||!result.success||!result.referral)throw new Error(result.message??"Response document could not be changed.");
      selectedReferral=result.referral;renderDetails(selectedReferral);await loadReferrals();showPageMessage(result.message??"Response document updated.","success");
    }catch(error:unknown){showPageMessage(error instanceof Error?error.message:"Response document could not be changed.","danger");}
  }
