...
if (patientUid) {
  ...
  async function load(): Promise<void> { root.setAttribute("aria-busy","true");root.innerHTML=`<div class="microemr-loading-state" role="status"><span class="microemr-loading-spinner" aria-hidden="true"></span><span>Loading tasks...</span></div>`;try{const response=await fetch(`/PatientTasks/List?patientUid=${patientUid}&status=${encodeURIComponent(filter.value)}`);const result=await response.json() as TaskResult;if(!response.ok)throw new Error(result.message??"Tasks could not be loaded.");tasks=result.tasks??[];render();}catch(error:unknown){root.replaceChildren();throw error;}finally{root.setAttribute("aria-busy","false");} }
