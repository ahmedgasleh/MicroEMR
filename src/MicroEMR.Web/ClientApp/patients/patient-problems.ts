...
function appendRequest(data: FormData, request: SaveProblemRequest | ResolveProblemRequest): void {
...
function initializePatientProblems(): void {
    ...
    if (!patientUid || !createUrl || !updateUrl || !resolveUrl || !refreshUrl) return;
    ...
    const tokenInput = document.querySelector<HTMLInputElement>('#problemAntiforgeryForm input[name="__RequestVerificationToken"]');
    ...
    const applyFilter = (): void => {
        ...
        let visibleCount = 0;
        document.querySelectorAll<HTMLTableRowElement>("tr[data-problem-status]").forEach(row => {
    ...
    addButton.addEventListener("click", () => {
        uidInput.value = ""; nameInput.value = ""; descriptionInput.value = ""; onsetInput.value = "";
        nameInput.classList.remove("is-invalid"); editorMessage.classList.add("d-none");
        editorTitle.textContent = "Add Problem"; editorModal.show();
    });

    document.querySelectorAll<HTMLButtonElement>(".edit-problem-button").forEach(button => button.addEventListener("click", () => {
        const row = button.closest<HTMLTableRowElement>("tr");
        if (!row) return;
        uidInput.value = row.dataset.problemUid ?? ""; nameInput.value = row.dataset.problemName ?? "";
        descriptionInput.value = row.dataset.problemDescription ?? ""; onsetInput.value = row.dataset.problemOnset ?? "";
        nameInput.classList.remove("is-invalid"); editorMessage.classList.add("d-none");
        editorTitle.textContent = "Edit Problem"; editorModal.show();
    }));

    document.querySelectorAll<HTMLButtonElement>(".resolve-problem-button").forEach(button => button.addEventListener("click", () => {
        const row = button.closest<HTMLTableRowElement>("tr");
        if (!row) return;
        resolvingUid = row.dataset.problemUid ?? null; resolveName.textContent = row.dataset.problemName ?? "";
        resolveReason.value = ""; resolveMessage.classList.add("d-none"); resolveModal.show();
    }));
    ...
    saveButton.addEventListener("click", async () => {
        ...
        saveButton.disabled = true; editorMessage.classList.add("d-none");
        ...
        try {
            const data = new FormData(); appendRequest(data, request);
            ...
            const result = await response.json().catch(() => ({})) as Partial<ProblemApiResponse>;
            if (!response.ok || !result.success) {
                showError(editorMessage, result.message ?? "Problem could not be saved."); return;
            }
            rememberFilterAndReload();
        } catch { showError(editorMessage, "Problem could not be saved."); }
        finally { saveButton.disabled = false; }
    });

    resolveButton.addEventListener("click", async () => {
        if (!resolvingUid) return;
        resolveButton.disabled = true; resolveMessage.classList.add("d-none");
        ...
        try {
            ...
            const result = await response.json().catch(() => ({})) as Partial<ProblemApiResponse>;
            if (!response.ok || !result.success) {
                showError(resolveMessage, result.message ?? "Problem could not be resolved."); return;
            }
            rememberFilterAndReload();
        } catch { showError(resolveMessage, "Problem could not be resolved."); }
