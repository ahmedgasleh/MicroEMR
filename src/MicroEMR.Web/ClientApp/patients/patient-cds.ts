...
interface CdsEvaluationEnvelope {
    success: boolean;
    message?: string;
    result?: { alerts: CdsAlert[]; rulesEvaluated: number; rulesFailed: number };
}

const root = document.querySelector<HTMLElement>("#patientCdsRoot");
const list = document.querySelector<HTMLElement>("#patientCdsList");
const message = document.querySelector<HTMLElement>("#patientCdsMessage");
const count = document.querySelector<HTMLElement>("#patientCdsCount");
const token = document.querySelector<HTMLInputElement>('#patientCdsAntiforgery input[name="__RequestVerificationToken"]');
...
function showMessage(text: string, style: "warning" | "danger" = "warning"): void {
    if (!message) return;
...
function renderAlert(alert: CdsAlert, baseUrl: string, canRespond: boolean): HTMLElement {
    ...
    card.append(action);
    card.append(element("div", "small text-body-secondary mb-3",
        `Rule ${alert.ruleKey} v${alert.ruleVersion}${alert.ruleSourceReference ? ` · ${alert.ruleSourceReference}` : ""}`));
    ...
    if (canRespond && alert.status === "Active") {
        ...
        acknowledge.addEventListener("click", () => void respond(`${baseUrl}/${alert.cdsAlertUid}/acknowledge`,
            { expectedRowVersion: alert.rowVersion }));
        controls.append(acknowledge);
    }
    if (canRespond && (alert.status === "Active" || alert.status === "Acknowledged")) {
        const reason = element("select", "form-select form-select-sm w-auto") as HTMLSelectElement;
        ...
        const comment = element("input", "form-control form-control-sm w-auto") as HTMLInputElement;
...
async function showHistory(card: HTMLElement, url: string): Promise<void> {
    const existing = card.querySelector<HTMLElement>(".cds-history");
    ...
    try {
        ...
        const payload = await response.json() as { success: boolean; items?: Array<{ eventType: string; occurredAtUtc: string; actorDisplayName?: string; reasonCode?: string }> };
        ...
        if (!container.childElementCount) container.textContent = "No lifecycle history is available.";
    } catch { container.textContent = "CDS history is temporarily unavailable."; }
}

async function respond(url: string, body: object): Promise<void> {
    const response = await postJson(url, body);
    if (!response.ok) {
        const payload = await response.json().catch(() => ({ message: "The CDS response could not be completed." })) as { message?: string };
...
async function load(): Promise<void> {
    ...
    try {
        ...
        const payload = await response.json() as CdsEvaluationEnvelope;
        if (!response.ok || !payload.success || !payload.result) throw new Error(payload.message);
        const alerts = payload.result.alerts ?? [];
        list.replaceChildren();
        count.textContent = String(alerts.length);
        for (const alert of alerts) list.append(renderAlert(alert, baseUrl, root.dataset.canRespond === "true"));
        if (!alerts.length) list.append(element("p", "text-body-secondary mb-0", "No active clinical decision support findings."));
        if (payload.result.rulesFailed > 0) showMessage("Some decision-support rules could not be evaluated. Existing findings were preserved.");
        else if (message) message.className = "alert d-none";
    } catch {
