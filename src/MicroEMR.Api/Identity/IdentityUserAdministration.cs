    ...
    ILogger<IdentityUserAdministration> logger) : IIdentityUserAdministration
{
        ...
        ResolveOrCreateIdentityRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (!result.Succeeded)
        {
            ...
            throw new ArgumentException("The Auth account could not be created. Verify the submitted identity information.");
        }
        logger.LogInformation("Passwordless Auth identity {UserId} created; account setup delivery remains pending.", user.Id);
