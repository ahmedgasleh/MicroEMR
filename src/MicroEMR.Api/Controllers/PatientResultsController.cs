...
public sealed class PatientResultsController(IPatientResultRepository repository, ILogger<PatientResultsController> logger) : ControllerBase
{
    ...
    [HttpPost] public Task<IActionResult> Create(Guid patientUid,CreatePatientResultRequest request,CancellationToken token)=>Mutate(()=>repository.Create(patientUid,request,UserId(),token),true);
    [HttpPut("{uid:guid}")] public Task<IActionResult> Update(Guid patientUid,Guid uid,UpdatePatientResultRequest request,CancellationToken token)=>Mutate(()=>repository.Update(patientUid,uid,request,UserId(),token));
    [HttpPost("{uid:guid}/mark-reviewed")] public Task<IActionResult> Review(Guid patientUid,Guid uid,MarkPatientResultReviewedRequest request,CancellationToken token)=>Mutate(()=>repository.Review(patientUid,uid,request,UserId(),token));
